<?php

/* NioutsMainBundle:Accueil:connexion.html.twig */
class __TwigTemplate_23a15abeaf99355078cb4c9318b7a51d8f7bbd6a55b73e0b8789e2c6f3b3df80 extends Twig_Template
{
    public function __construct(Twig_Environment $env)
    {
        parent::__construct($env);

        $this->parent = $this->env->loadTemplate("NioutsMainBundle::layout.html.twig");

        $this->blocks = array(
            'title' => array($this, 'block_title'),
            'css' => array($this, 'block_css'),
            'body' => array($this, 'block_body'),
        );
    }

    protected function doGetParent(array $context)
    {
        return "NioutsMainBundle::layout.html.twig";
    }

    protected function doDisplay(array $context, array $blocks = array())
    {
        $this->parent->display($context, array_merge($this->blocks, $blocks));
    }

    // line 3
    public function block_title($context, array $blocks = array())
    {
        $this->displayParentBlock("title", $context, $blocks);
        echo " - ";
        echo twig_escape_filter($this->env, $this->env->getExtension('translator')->trans("connexion.titre"), "html", null, true);
    }

    // line 5
    public function block_css($context, array $blocks = array())
    {
        // line 6
        $this->displayParentBlock("css", $context, $blocks);
        echo "

";
    }

    // line 11
    public function block_body($context, array $blocks = array())
    {
        // line 12
        echo "    <h2>";
        echo twig_escape_filter($this->env, $this->env->getExtension('translator')->trans("connexion.titre"), "html", null, true);
        echo "</h2>
    
    <form method=\"post\" action=\"";
        // line 14
        echo $this->env->getExtension('routing')->getPath("_connexionCheck");
        echo "\">
\t\t<!--  Code -->
\t    <div class=\"formLigne\">
\t\t\t<div class=\"formLabel\"><label for=\"username\" class=\"required\">";
        // line 17
        echo twig_escape_filter($this->env, $this->env->getExtension('translator')->trans("connexion.form.label.code"), "html", null, true);
        echo "</label></div>
\t\t\t<div class=\"formSaisie\">
\t\t\t\t<input type=\"text\" id=\"username\" name=\"_username\" value=\"\" />
\t\t\t</div>
\t\t</div>
\t\t
\t\t<!--  Mot de Passe -->
\t    <div class=\"formLigne\">
\t\t    <div class=\"formLabel\"><label for=\"password\" class=\"required\">";
        // line 25
        echo twig_escape_filter($this->env, $this->env->getExtension('translator')->trans("connexion.form.label.pass"), "html", null, true);
        echo "</label></div>
\t\t\t<div class=\"formSaisie\">
\t\t\t\t<input type=\"password\" id=\"password\" name=\"_password\" />
\t\t\t</div>
\t\t</div>
\t\t
\t\t<input type=\"hidden\" name=\"_target_path\" value=\"/accueil\" />
\t\t
\t\t<div class=\"formLigneBouton\">
\t\t\t<input type=\"submit\" value=\"";
        // line 34
        echo twig_escape_filter($this->env, $this->env->getExtension('translator')->trans("connexion.form.label.submit"), "html", null, true);
        echo "\" class=\"bouton\"/>
\t\t</div>\t\t
\t</form> 
\t
";
    }

    public function getTemplateName()
    {
        return "NioutsMainBundle:Accueil:connexion.html.twig";
    }

    public function isTraitable()
    {
        return false;
    }

    public function getDebugInfo()
    {
        return array (  134 => 16,  110 => 50,  100 => 42,  84 => 40,  23 => 1,  58 => 14,  480 => 162,  474 => 161,  469 => 158,  461 => 155,  457 => 153,  453 => 151,  444 => 149,  440 => 148,  437 => 147,  435 => 146,  430 => 144,  427 => 143,  423 => 142,  413 => 134,  409 => 132,  407 => 131,  402 => 130,  398 => 129,  393 => 126,  387 => 122,  384 => 121,  381 => 120,  379 => 119,  374 => 116,  368 => 112,  365 => 111,  362 => 110,  360 => 109,  355 => 106,  341 => 105,  337 => 103,  322 => 101,  314 => 99,  312 => 98,  309 => 97,  305 => 95,  298 => 91,  294 => 90,  285 => 89,  283 => 88,  278 => 86,  268 => 85,  264 => 84,  258 => 81,  252 => 80,  247 => 78,  241 => 77,  229 => 73,  220 => 70,  214 => 69,  177 => 65,  169 => 60,  140 => 55,  132 => 51,  128 => 49,  107 => 36,  61 => 13,  273 => 96,  269 => 94,  254 => 92,  243 => 88,  240 => 86,  238 => 85,  235 => 74,  230 => 82,  227 => 81,  224 => 71,  221 => 77,  219 => 76,  217 => 75,  208 => 68,  204 => 72,  179 => 69,  159 => 61,  143 => 56,  135 => 53,  119 => 42,  102 => 32,  71 => 19,  67 => 30,  63 => 17,  59 => 14,  38 => 5,  94 => 28,  89 => 20,  85 => 25,  75 => 35,  68 => 14,  56 => 9,  87 => 25,  21 => 2,  26 => 2,  93 => 41,  88 => 6,  78 => 21,  46 => 7,  27 => 4,  44 => 12,  31 => 5,  28 => 3,  201 => 92,  196 => 90,  183 => 82,  171 => 61,  166 => 71,  163 => 62,  158 => 67,  156 => 66,  151 => 50,  142 => 59,  138 => 27,  136 => 56,  121 => 46,  117 => 44,  105 => 40,  91 => 27,  62 => 27,  49 => 19,  24 => 4,  25 => 3,  19 => 1,  79 => 18,  72 => 16,  69 => 25,  47 => 9,  40 => 10,  37 => 10,  22 => 2,  246 => 90,  157 => 56,  145 => 30,  139 => 45,  131 => 15,  123 => 58,  120 => 40,  115 => 43,  111 => 37,  108 => 36,  101 => 32,  98 => 31,  96 => 31,  83 => 25,  74 => 25,  66 => 15,  55 => 15,  52 => 21,  50 => 17,  43 => 8,  41 => 6,  35 => 8,  32 => 4,  29 => 3,  209 => 82,  203 => 78,  199 => 67,  193 => 73,  189 => 71,  187 => 84,  182 => 66,  176 => 64,  173 => 65,  168 => 72,  164 => 59,  162 => 57,  154 => 51,  149 => 51,  147 => 58,  144 => 49,  141 => 28,  133 => 55,  130 => 41,  125 => 44,  122 => 43,  116 => 54,  112 => 52,  109 => 34,  106 => 36,  103 => 32,  99 => 31,  95 => 28,  92 => 21,  86 => 34,  82 => 39,  80 => 19,  73 => 19,  64 => 29,  60 => 6,  57 => 14,  54 => 10,  51 => 12,  48 => 11,  45 => 17,  42 => 7,  39 => 9,  36 => 5,  33 => 4,  30 => 3,);
    }
}
